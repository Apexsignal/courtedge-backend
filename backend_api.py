...
@app.get("/admin/debug-full-history")
def debug_full_history(_: None = Depends(require_admin_key)) -> list[dict]:
    """Dočasný diagnostický endpoint — appka dělá kompletní rozbor VŠECH
    appčiných favorite tiketů od začátku appčiny historie (ne jen
    posledních pár dní), hledá vzorec napříč appka Elo, jistotou,
    tržním kurzem, turnajem, surface a časem výkopu. Smaže se hned
    po analýze."""
    with db.get_cursor() as cur:
        cur.execute(
            """
            SELECT t.id AS ticket_id, t.status AS ticket_status, t.created_at AS ticket_created_at,
                   ...
                   m.surface, m.tourney_level, m.tourney_name, m.start_time, m.retirement, m.tour
            FROM tickets t
        ...
        for r in rows:
            r["ticket_created_at"] = r["ticket_created_at"].isoformat()
